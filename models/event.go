...
func (e Evento) Update() error {
	query := `
	UPDATE eventos
	SET nome = $1, descricao = $2, local = $3, dateTime = $4
	WHERE id = $5
`
	stmt, err := db.DB.Prepare(query)
	if err != nil {
	...
	_, err = stmt.Exec(e.Nome, e.Descricao, e.Local, e.Datetime, e.Id)
